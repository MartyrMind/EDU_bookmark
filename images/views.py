...
def image_create(request):
    if request.method == 'POST':
        ...
        # для создания экземпляра формы необходимо предоставить начальные данные через HTTP-запрос методом GET
        # данные - url и title изображения с внешнего сайта - задаются букмарклетом JS
...
def image_detail(request, id_, slug):
    ...
    # cчитаем количество просмотров
    total_views = r.incr(f'image:{image.id}:views')
    # считаем рейтинг изображения
